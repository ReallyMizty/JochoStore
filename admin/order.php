			...
			if (isset($_POST['add']))
				{
					...
					$code = rand(0,98987787866533499);
								...
								{
									if($size > 20000) //conditions for the file
									{
...
<div id="leftnav">
	...
	<div id="rightcontent" style="position:absolute; top:10%;">
			<div class="alert alert-info"><center><h2>ออเดอร์</h2></center></div>
			<br />
				<div style='width:975px;' class="alert alert-info">
					  <table class="table table-hover">
						<thead>
						  ...
						  <tbody>
							<?php
							$Q1 = $conn->query("SELECT * FROM transaction WHERE order_stat = 'ชำระเงินแล้ว'");
							...
							$pid = $r2['product_id'] ?? null;
                            $o_qty = $r2['order_qty'] ?? null;
                            $p_price = $r2['product_price'] ?? null;
                            $brand = $r2['product_name'] ?? null;
							...
							$Q3 = $conn->query("SELECT sum(amount) FROM transaction WHERE order_stat = 'ชำระเงินแล้ว'");
